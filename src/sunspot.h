...
// how are input and output values encoded?
LIBEA_MD_DECL(SUNSPOT_ENCODING, "sunspot.encoding", std::size_t);
...
struct sunspot_fitness : fitness_function<unary_fitness<double>, constantS, stochasticS> {
    ...
    int _encoding; //! Cached encoding type.
    ...
    void initialize(RNG& rng, EA& ea) {
        ...
        _encoding = get<SUNSPOT_ENCODING>(ea,0);
    }
    
    //! Encode integer value x.
    int encode(int x) {
        switch(_encoding) {
            case 1: { // reflected binary gray code
                return (x >> 1) ^ x;
            }
            default: return x; // no encoding
        }
    }

    //! Decode integer value x.
    int decode(int x) {
        switch(_encoding) {
            case 1: { // reflected binary gray code
                for(int mask=(x >> 1); mask!=0; mask = mask >> 1) {
                    x = x ^ mask;
                }
                return x;
            }
    ...
    void load_file(const std::string& filename, matrix_type& input, vector_type& t, vector_type& tplus1, EA& ea) {
        ...
        for(std::size_t i=0; i<smat.size(); ++i) {
            // we just need the 2nd column -- the first is an index:
            std::string& s = smat[i][IDX_X];
            long x=0;

            if(regex_search(s, decimal)) {
                // if we have a decimal in the string, we assume that we're reading a double:
                x = static_cast<long>(boost::lexical_cast<double>(s) * factor);
            } else {
                // it's already an integer; we're done
                x = boost::lexical_cast<long>(s) << get<SUNSPOT_FRACTIONAL_BITS>(ea);
            }
        ...
        for(std::size_t i=0; i<imat.size1(); ++i) {
            for(std::size_t j=0; j<imat.size2(); ++j) {
                std::bitset<sizeof(long)*8> bits(encode(imat(i,j)));
	...
	dvector_type eval(Individual& ind, matrix_type& output, matrix_type& input, vector_type& observed, RNG& rng, EA& ea) {
        ...
        for(std::size_t i=0; i<input.size1(); ++i) {
            ...
            for(std::size_t j=0; j<ph; ++j) {
                output(i,j) = decode(algorithm::range_pair2int(N.begin_output()+j*2*nbits,
                                                               N.begin_output()+(j+1)*2*nbits));
            }
        }
